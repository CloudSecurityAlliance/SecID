...
# AICM's 18 domain codes (16 shared with CCM, I&S in place of IVS, plus MDS).
# Item patterns under version nodes must enumerate them: a shape pattern such
# as ^[A-Z&]{2,3}$ fails the pattern-breadth gate and fabricates matches.
AICM_DOMAINS = "(?:A&A|AIS|BCR|CCC|CEK|DCS|DSP|GRC|HRS|I&S|IAM|IPY|LOG|MDS|SEF|STA|TVM|UEM)"
AICM_DOMAIN = f"^{AICM_DOMAINS}$"
AICM_CONTROL = f"^{AICM_DOMAINS}-\\d{{2}}$"
AICM_QUESTION = f"^{AICM_DOMAINS}-\\d{{2}}\\.\\d+$"
...
def test_aicaiq_version_nodes_carry_all_three_item_levels():
    for v in ("1.1.0", "1.0.2"):
        ...
        assert AICM_DOMAIN in pats, (v, pats)
        assert AICM_CONTROL in pats, (v, pats)
        assert AICM_QUESTION in pats, (v, pats)
        assert not any("[A-Z&]" in p for p in pats), (v, pats)
