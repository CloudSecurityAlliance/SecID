...
**SecID 2.0 is in design.** v1.0 answers *where is it*; v2.0 answers *how do I get it, what will it cost me, how often does it change, and can I verify what you gave me* — serving the content itself where licensing permits. Crucially for registry work: **v2.0 adds nothing to the registry format.** The registry keeps holding identity, resolution, and disambiguation, and refers out to separate `SecID-Data-*` repositories (ADR-010).

See ROADMAP.md ("SecID 2.0 — From Pointers to Data") for goals, GOALS.md for outcomes, and DECISIONS.md ADR-010/011/012 for the decisions and their rejected alternatives.
...
| **SecID-Data-\*** | Extracted content, sharded by the authority behind it — `International`, `North-America`, `Latin-America`, `Europe`, `Asia`, `Middle-East`, `Africa`, `Oceania`, `Staging` (planned, v2.0). Private counterparts with identical names live under `CloudSecurityAlliance-Internal`. |
