...
See [DECISIONS.md ADR-015](../../DECISIONS.md) for the decision and its rejected alternatives.
...
- [ ] **Tracking aliases** (`v1` → latest `1.y.z`). A source-level `version_tracks` field, *not* an `aliases[]` entry — nesting fixes the target, so a moving pointer cannot live there. A track collapses into a fixed alias once its major line closes (`CCM v3` → `3.0.1`). Deliberately unbuilt: moving pointers are the hazard ADR-015 addresses; Maven removed `LATEST` and Debian warns against `stable` for the same reason. **One manual exception exists:** AICM `1.1` / `v1.1` point at `1.1.1`, the newest 1.1.x, because CSA's own csa-mcp server resolves the series label that way (ADR-015 amendment, 2026-09-25). It is an ordinary `aliases[]` entry that has to be moved by hand when CSA ships a newer 1.1.x — if a second such case appears, build `version_tracks` instead of repeating the exception.
...
- [ ] **AICM `1.1.1` has no asserted release date and no distinct artifact URL.** CSA's v1.1 artifact page still reads "Released: 06/22/2026" (1.1.0's date) without naming a patch level, and the anonymous download URL now redirects to that page rather than serving the ZIP, so the patch level in the current bundle could not be confirmed directly. The 1.1.1 record rests on CSA's csa-mcp dataset. Revisit when CSA publishes a changelog or a 1.1.1-specific page.
